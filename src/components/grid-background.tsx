...
function Stars() {
  ...
  const [positions, colors, sizes] = useMemo(() => {
    ...
    for (let i = 0; i < STAR_COUNT; i++) {
      ...
      // Distribute stars in a sphere with galaxy-like density
      ...
      // Varying star colors - white, blue-white, pale yellow
      ...
      if (colorChoice < 0.5) {
        ...
        // Pale cyan stars (matching theme)
        colors[i3] = 0.24
        colors[i3 + 1] = 0.61
        colors[i3 + 2] = 0.72
      } else {
...
function Nebula() {
  ...
  const [positions, colors] = useMemo(() => {
    ...
    for (let i = 0; i < NEBULA_PARTICLES; i++) {
      ...
      // Spiral galaxy arm distribution
      ...
      // Nebula colors - teal/cyan theme with purple accents
      const colorMix = Math.random()
      if (colorMix < 0.4) {
        // Teal (matching theme #3d9cb7)
        colors[i3] = 0.24
        colors[i3 + 1] = 0.61
        colors[i3 + 2] = 0.72
